...
const steps = [
  ...
  { name: "Messages", func: require("./migrations/migrateMessages") },
  {
    ...
    func: require("./migrations/migrateInternalMessages"),
  }
];

async function run() {
  const ctx = buildContext();
  const scopeKey = ctx.isSingleTenant
    ? `tenant-${ctx.tenantId}`
    : "all-tenants";
  ...
  if (fs.existsSync(progressPath)) {
    try {
      ...
      lastCompletedIndex = steps.findIndex(
        (s) => s.name === progress.lastCompleted,
      );
    } catch (e) {
      console.log(
        chalk.yellow(
          "⚠️  Não foi possível ler o progress anterior. Reiniciando do início.",
        ),
      );
    }
  }
  ...
  for (let i = lastCompletedIndex + 1; i < steps.length; i++) {
    ...
    if (!success) {
      console.log(
        chalk.red(`❌ Parando execução. Corrija o erro e reexecute.`),
      );
      process.exit(1);
    }
